...
4. Socket機能
   ...
   4. ルームにアクセス時にソケットを接続に変更
   5. Socket機能を一つにまとめて分離
5. サインイン機能
   1. ⭕サインインフォーム作成
   2. ⭕サインインユーザーを登録
   3. ⭕サインアウト処理
...
8. ルーム機能
   1. ⭕テーブル用意
   2. カスケードデリート設定
   3. ルームアクセス時にSocket接続を開始
   4. ルームの作成
   5. ルームID一致したチャットリスト取得
...
9. レイアウト調整
   ...
   4. プロフィールフォーム画面
   5. TOPのルーム一覧
   6. 投稿が自分か他人か判定してレイアウト変える
   7. 編集・削除するときのアラートをやめてステータス通知表示（エラー表示と合わせて）
   8. エラーのアラートをやめてステータス通知表示（エラー表示と合わせて）
   9. チャット更新の度に最新にスクロール移動
   10. チャットの入力フォームが最初にフォーカスするように
   11. キーボードタブでのフォーカスの可視化（アクセシビリティ）
10. README
    1. 関連ファイルをフォルダにまとめる
    2. ER図のキャプチャ画像を貼る
    3. ❎参考URLをタブ分け
...
- チャットページの初回アクセスのみDBからデータ取得してJotaiでステート管理、ページにいる間はすべてステート更新だけでリアルタイムに更新させた
  - DB更新と一緒にステート更新だけでなくソケットに更新内容を受信して再度ステート更新させてる
    - （結局reactQueryのクエリ更新もさせてるけど使ってない…。）
