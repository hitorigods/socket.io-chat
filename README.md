...
6. チャット機能
   ...
   8. バリデーション（Zod）
   9. テストコード実装
   10. ❎受信したいデータのセレクト方法をバックエンド側で処理（Supabaseカスタムフック？）
   11. ❎チャットの非公開設定
   12. ❎チャットが更新されたら音鳴らす
   13. ❎他ユーザーのサインイン通知、入退室が目に見えるように
7. プロフィール機能
   1. ⭕テーブル用意
   2. ⭕初回にプロフィール情報を登録
   3. ⭕プロフィール情報を編集
...
8. ルーム機能
   ...
   2. ⭕ルームの作成
   3. ⭕作成したユーザーのみルーム編集
   4. ⭕ルームIDに一致したチャットリストを取得
   5. ⭕ルームの一覧表示と個別ページにルーティング
   6. 同一ルームIDのもののみSocket受信
   7. カスケードデリート設定
   8. RLS設定（閲覧は全ユーザー、編集削除は所有ユーザーのみ）
   9. バリデーション（Zod）
   10. テストコード実装
   11. ❎ルームアクセス時にSocket接続して直リンク可能に
9. レイアウト調整
...
12. 💀💀💀 **BugFix** 💀💀💀
    1. ⭕デプロイ先でソーシャルログインしたらリダイレクト先がlocalhostになる
       1. → ⭕Supabaseクライアントの認証→URL設定忘れ
    2. ⚠デプロイ先でストレージにアップロードした画像がリンク切れになる
       1. → ⭕Supabase外部URL許可とRenderのImageコンポーネント処理でメモリ不足が発生
       2. ❎↑代案考える（アップロードサイズ制限？アップロード時にリサイズ登録できないか？）
