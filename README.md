...
- チャットページに入ったらDBの最新データの取得は初回のみステート管理（Jotai）してページにいる間はすべてステート更新だけでリアルタイムに更新させた
...
1. 画面実装
   ...
   13. エラー表示
       1. アラート内容を日本語に
       2. アラートではなく通知表示に
   14. atomコンポーネントをコロケーション分離
   15. Socket機能を一つにまとめて分離
2. データベース
   1. ⭕ORM（prisma）導入
   2. ⭕Supabase用意
...
3. 認証機能
   ...
   2. OAuth実装
      1. Google
4. レイアウト調整
   1. サインイン/プロフィールフォームの体裁
   2. ⭕ナビゲーション（TOP/プロフィール/サインアウト/チャット一覧）
